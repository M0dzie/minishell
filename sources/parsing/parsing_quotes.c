...
/*   Updated: 2023/02/20 16:20:41 by thmeyer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	word_len_in_quotes(t_msl *ms, char *input, int i)
{
	int	len;
...
char	*del_quotes(t_msl *ms, char *input, int i, char c)
{
	...
	j = -1;
	len = word_len_in_quotes(ms, input, i) + 1;
	printf("len = %d\n", len);
	...
	if (!new)
		return (NULL);
	printf("input = %s\n", input + i);
	ft_strlcpy(new, input + i, len + 1);
	printf("new1 = %s\n", new);
	new = ft_strtrim(new, &c);
	if (!new)
		return (NULL);
	new = ft_strjoin(new, input + ms->lst_quote + 1);
	if (!new)
		return (NULL);
	printf("new2 = %s\n", new);
	return (new);
...
int	parsing_quotes(t_msl *ms)
{
	...
	while (input[++i])
	{
		if (input[i] == 34 || input[i] == 39)
		{
			if (check_opened_quotes(ms, input, i + 1, input[i]))
				return (display_errors(ms, input[i]));
			printf("len_word = %d\n", word_len_in_quotes(ms, input, i - 1));
			input = del_quotes(ms, input, i, input[i]);
			i = ms->lst_quote - 2;
		}
	}
	ms->input = input;
